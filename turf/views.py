...
def orderHistory(request):
    ...
    bookings = TurfBooked.objects.filter(email=request.user.email)
    # bookedSlots = []
    ...
    return render(request, 'orderHistory.html', {'bookings': bookings, 'currentDate': currentDate})
...
def allBookings(request):
    datesInSortedOrder = []
    bookings = TurfBooked.objects.all().order_by('selected_date', 'booking_time')
    currentDate = date.today().strftime("%Y-%m-%d")
